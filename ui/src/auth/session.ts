 ...
 * can stay thin. Every password-derived computation happens here on the client; the
...
export async function recoverAccountWithMnemonic(input: RecoverInput): Promise<void> {
...
/** Set (or replace) the destruction password verifier. Entering this password at login silently wipes the account. */
...
/** Permanently delete the account (cascades server-side), then wipe local state. Used by the nuke button in Settings > Security */
