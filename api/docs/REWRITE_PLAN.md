...
All FKs `ON DELETE CASCADE` (parity with current). `created_at`/`updated_at` stay plaintext per the metadata decision; the _meaningful_ date (the tracked day) is encrypted in `enc_date`.
